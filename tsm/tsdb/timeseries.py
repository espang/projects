...
def write_data(conn, pk, series):
    # key to the hash of timeseries
    hash_key = _timeseries_hash_key(pk=pk)
    # key to set of years with values
    set_key = _timeseries_block_starts(pk=pk)
    # read ts attributes
    ts = read_ts(conn, pk)
